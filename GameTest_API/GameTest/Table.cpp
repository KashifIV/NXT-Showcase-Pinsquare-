...
bool CLineSegment::IsOnLine(CLineSegment val, float tolerance)
{
	const float x00 = val.m_start.m_x;
	const float x11 = val.m_end.m_x;
	const float y00 = val.m_start.m_y;
	const float y11 = val.m_end.m_y;
	return DistanceToLine(x11 - x00, y11 - y00) < tolerance || DistanceToLine(x00, y00) < tolerance || DistanceToLine(x11, y11) < tolerance;
}
